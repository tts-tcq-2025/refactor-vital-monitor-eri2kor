...
// Named constant for default warning tolerance
...
BreachType checkLimit(float value, const Limit& limit) {
    if (value < limit.min) {
        return BreachType::LOW;
    }
    if (value > limit.max) {
        return BreachType::HIGH;
    }
...
BreachType checkLimitWithWarning(float value, const Limit& limit, float tolerance) {
    ...
    if (value <= limit.min + tol) {
        return BreachType::WARNING_LOW;
    }
    if (value >= limit.max - tol) {
        return BreachType::WARNING_HIGH;
    }
    return BreachType::NORMAL;
}
...
int vitalsOk(const std::vector<Vital>& vitals) {
    ...
    for (const auto& v : vitals) {
        BreachType breach = checkLimitWithWarning(v.value, v.limit, DEFAULT_WARNING_TOLERANCE);
